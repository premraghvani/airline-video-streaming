...
module.exports = {
  ...
  execute: async (req, res) => {
    ...
    if (end >= totalSize) {
      res
        .status(400)
        .send({ message: "Invalid content range - end can not exceed size" });
      return;
    }
    if (start > end) {
      res
        .status(400)
        .send({
          message: "Invalid content range - start can not be after end",
        });
      return;
    }
    ...
    if (chunkSize > 2 * 10 ** 6) {
      res
        .status(400)
        .send({ message: "Invalid - the chunk exceeds 2MB (2,000,000 bytes)" });
      return;
    }
    ...
    if (bodySize != chunkSize) {
      res
        .status(400)
        .send({
          message: `Invalid - the chunk size implied in header (${chunkSize} B) does not match the size of the body (${bodySize} B)`,
        });
      return;
    }
