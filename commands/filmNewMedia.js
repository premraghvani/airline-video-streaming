...
module.exports = {
    ...
    execute: async(req, res) => {
        ...
        if(!rangeMatch){
            res.status(400).send({message:"Invalid content range"});
            return;
        }
        ...
        if(end >= totalSize){
            res.status(400).send({message:"Invalid content range - end can not exceed size"});
            return;
        }
        if(start > end){
            res.status(400).send({message:"Invalid content range - start can not be after end"});
            return;
        }
        ...
        if(chunkSize > 2* 10**6){
            res.status(400).send({message:"Invalid - the chunk exceeds 2MB (2,000,000 bytes)"});
            return;
        }
        ...
        if(bodySize != chunkSize){
            res.status(400).send({message:`Invalid - the chunk size implied in header (${chunkSize} B) does not match the size of the body (${bodySize} B)`});
            return;
        }
        ...
        if(movieMetadata.acceptingChange == undefined){
            res.status(400).send({message:"Can't upload data to this movie."});
            return;
        ...
        try {
            ...
            // conclusion
            res.status(200).send({message: "Part uploaded successfully"});
            return;

        } catch(err){
            // errors
            res.status(500).send({message: "File write error"});
            return;
        }
    }
};
